...
> 本地个人助手 Agent 工作台 — 在你的电脑上运行、管理和持续使用一个可执行的专用 Agent

## 产品定位

Agent-Smith 把 AI Agent 收口为一个可长期协作的「个人助手 Agent」。它运行在本机环境中，围绕你的文件、任务、知识、技能、权限和工作记录持续工作，而不是每次从零开始的一次性聊天。

**不是聊天机器人，不是云端壳**——用户使用的是一个持续存在、可执行、可积累的本地 Agent，而不是一次性 prompt。

> 过渡说明：当前 server / app / 数据目录里仍保留 `employee` / `employees` 命名，用来兼容现有前端和数据模型；这次先收口产品语义、默认模板和 CLI，不改前端壳。
...
| `agents/` | Agent 内容定义（9 个模板、6 个内置技能、9 个工具、24 条安全规则） | .md/.yaml/.json + Python |
| `server/` | 平台后端（FastAPI，Agent 实例/会话/任务/团队管理，23+ API 端点） | Python |
...
先进入 `server/`，CLI 会直接复用现有 employee/session/engine 服务，但对外体验默认收口为一个内置个人助手 Agent：

```bash
cd server && uv sync
uv run python -m app.cli agent ensure
uv run python -m app.cli agent show
uv run python -m app.cli sessions list
uv run python -m app.cli chat --message "帮我分析这个仓库的登录流程" --verbose
```

不传 `--message` 时会进入交互式 REPL：
...
cd server
uv run python -m app.cli sessions list
uv run python -m app.cli sessions show <session-id>
uv run python -m app.cli chat --session <session-id>
uv run python -m app.cli chat --session <session-id> --message "继续上次的任务"
```
...
│   ├── filesystem.py        #   Agent 实例目录操作（兼容保留 employees/ 命名）
│   ├── log.py               #   日志
...
│
├── agents/                  # Agent 内容定义
│   ├── templates/           #   9 个模板（只读，Git 跟踪；CLI 默认使用 personal-assistant）
...
默认内置一个 `personal-assistant` 模板，创建后成为长期存在的本地 Agent 实例，拥有自己的提示层、技能、记忆和权限。底层实例目录当前仍位于 `~/.agent-smith/employees/<id>/`，但产品语义已经收口为“个人助手”。
...
四层覆盖，优先级从低到高：环境变量/平台级 → 模板级 → Agent 实例级（目录仍名为 `employees/`） → 会话级。
