...
**agent-Smith 是跑在你电脑上的数字员工管理与任务执行台。**

它不是一个单纯聊天窗口，也不是云端知识库的前端壳；它把 Agent 产品化成一个个可管理的「数字员工」。每个数字员工都有角色、人设、头像、记忆、技能、MCP/连接器、权限、任务和工作记录，并运行在本机环境里，替用户处理对话任务、自动任务和本地工具调用。

## 当前产品形态

当前原型以 QoderWake 的数字员工产品形态为参考，但不复用其代码、品牌或素材；Agent-Smith 保留自己的技术栈和产品边界。
...
1. **Agent 是数字员工，不是模型聊天框**：用户管理的是可长期存在的员工，而不是一次性 prompt。
...
5. **工作过程可追踪**：员工首页必须有工作记录、入职信息、记忆与积累，避免 Agent 变成不可审计的黑箱。

## 与 AI Knowledge Hub 的边界

| | agent-Smith（本地） | AI Knowledge Hub（云端） |
|---|---|---|
| 定位 | 数字员工、任务、记忆、本地工具、权限 | 团队知识库、Wiki、权限、检索、MCP token |
| 数据归属 | 个人电脑（`~/.agent-smith`） | 团队服务器 |
| 核心对象 | 数字员工 / 任务 / 技能 / 连接器 / 记忆 | 知识库 / 文档 / Wiki / 用户权限 |
