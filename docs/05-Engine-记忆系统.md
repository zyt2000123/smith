...
记忆按当前 YAML 领域身份隔离：`~/.agent-smith/agent/identity-state/<identity-id>/memory/`。Smith 仍是唯一运行中的 Agent；隔离的是领域状态与 checkpoint，而不是新建多个 Agent 档案。
...
| NFR-2 | **token 有界**：被动注入的记忆量必须有上限 | recent ≤8K、durable ≤10K 字符（代码硬截断）；episode 检索最多 3 篇、总量 ≤6K 字符 |
...
```
                写入                     编译(LLM蒸馏+指纹缓存)            注入
对话结束 ──▶ recent.jsonl(唯一事件源) ──┬──▶ recent.md   (7-14天,≤8K)──┐
...
| 短时记忆 | `recent.md` | 时效性强（7-14 天窗口），周期重编译 |
...
| Legacy 条目存储 | `store.py` → `FileMemoryStore` | 为 memory_ops 保留的条目 CRUD |
...
**写入内容**：`recent.jsonl` 追加一行 `{"task": user_msg, "summary": reply, "timestamp": ISO-8601}`——作为编译的**唯一原料**。单字段超过 16K 时才截断（保留首尾，显式标记），普通对话完整保留。

不再写散落 .md 条目。`FileMemoryStore` 保留供 `memory_ops` 工具使用（Legacy 兼容）。
...
| 目标 | 输入 | 输出约束 | 累积方式 |
|---|---|---|---|
| `compile_recent` | 近 7-14 天流水（弹性窗口） | `recent.md`，≤8K 字符（代码硬截断） | 每次重建 |
| `compile_durable` | 最近 20 条流水 + 旧 durable.md | `durable.md`，≤10K 字符（代码硬截断） | **增量合并式**：新事件 merge 进既有长期记忆，LLM 负责去重和冲突解决 |
...
| 步骤 | 算法 | 设计依据 |
|---|---|---|
| 清秘 | 8 组正则（`sk-…`、`api_key=`、`password=`、`ghp_…`、`PRIVATE KEY` 等）逐行扫描，命中即删行 | NFR-6：密钥进了记忆 = 每次注入 prompt 都在泄漏。无 LLM 调用，快速确定性 |
...
**索引结构**（`search.sqlite`，WAL 模式）：
- `memory_fts`：FTS5 虚拟表（`entry_id, content, scope`，unicode61 分词）——FTS5 内建于 SQLite，零外部依赖
...
**使用场景**：`search_relevant_memories()` 在组装 prompt 前按用户消息检索 episodes 目录下的 `.md` 文件，每次动态索引再检索，最多返回 3 篇、总量 ≤6K 字符。失败静默返回空串（NFR-1）。

`memory_ops` 工具的 search 走 `FileMemoryStore` 关键词扫描（Legacy 路径），不经过 FTS5。
...
`memory_ops` 工具暴露 search/add/update/remove。Agent 在 ReAct 循环中自己判断"这个问题我是不是记过什么"，这对应人类的**主动回忆**行为。工具通过 `FileMemoryStore`（Legacy）读写散落 `.md` 条目文件，与主写入链路（`recent.jsonl`）是**两套独立存储**。主链路的 `assemble_memory()` 只读编译产物，不读散落条目；`memory_ops` 的 search 走关键词扫描，不走 FTS5。
...
| 文件 | 格式 | 字段 | 生产者 → 消费者 |
|---|---|---|---|
| `memory/recent.jsonl` | JSONL | `task` / `summary` / `timestamp`(ISO-8601 UTC)；单字段 >16K 时截断保留首尾 | save_conversation_memory → compile |
| `memory/agent/*.md`、`memory/project/*.md` | frontmatter + 正文 + Evidence | `id`(12位hex) / `scope` / `created_at` / `last_accessed` | Legacy: memory_ops 读写 |
...
| `memory/.fp_recent`、`.fp_durable` | 纯文本 MD5 | 指纹 | compile（自产自销） |
...
| `memory/search.sqlite` | SQLite（FTS5 虚拟表） | 见 4.4 | SearchIndex |
...
- **决策**：(d)——当前实现为 recent.md（近期活动，7-14 天窗口）+ durable.md（增量合并的长期事实）+ episodes（按主题归档）
...
- **理由**：三轨回答的是不同问题。durable 回答"这个用户总体是谁"，recent 回答"近期在做什么"，episodes 回答"和这句话有关的旧经验是什么"。只有摘要则记忆与当前任务无关；只有检索则丢失全局画像——多轨是刻意的。成本可控：编译产物有代码级硬截断（recent ≤8K、durable ≤10K 字符），episodes 检索最多 3 篇、≤6K 字符。
...
- **代价**：FTS5 的 unicode61 分词对中文按字切分，词组精度低于专业中文分词。向量语义检索路径待未来按需引入。
...
| AC-8 | 被动注入 token 有界：recent ≤8K + durable ≤10K 字符硬截断 + episode 检索 top-3、≤6K 字符 | 代码核实 `compile.py` 常量 + `search_relevant_memories` 截断 |
...
| 局限 | 影响 | 演进 |
|---|---|---|
| `memory_ops` 与主链路双轨存储 | 工具走 FileMemoryStore 写散落 .md，主链路走 recent.jsonl；两套存储互不可见 | 统一到 recent.jsonl，或让 memory_ops 写入也追加事件 |
| `project` 域无自动写入口 | 跨 Agent 共识层空转 | 在编译层增加"项目事实"判别，或团队会话落 project 域 |
| FTS5 unicode61 分词对中文按字切分 | 中文词组精度低 | 接入专业中文分词或增加向量检索路径 |
