...
| Context / Memory | [`PromptAssembler`](../../engine/prompt/assembler.py) 分层组装身份、工作流、工具、Skill、记忆和运行时上下文；[`FileMemoryStore`](../../engine/memory/store.py) 支持 agent/project scope 与检索 | 方向符合主流；下一步是明确 context、session、durable memory 的生命周期和评测 |
