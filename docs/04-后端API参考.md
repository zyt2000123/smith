...
**流程：**
1. 生成 ID：`uuid4().hex[:8]`
2. 在数据库中创建 Agent 记录（`device` 默认为当前主机名）
...
更新 Agent 信息。仅更新请求体中提供的字段，未提供的字段保持不变。
...
删除 Agent。同时删除数据库记录和 `~/.agent-smith/employees/<id>/` 目录下的所有文件。
