...
操作 Agent 实例目录（`~/.agent-smith/employees/<id>/`）下的配置文件。
...
### Agent `schemas/agent_profile.py` / `schemas/employee.py`
...
| `EmployeeService` | `employee_service.py` | `list_employees`, `get_employee`, `create_employee`（DB + 初始化 profile 文件）, `update_employee`, `delete_employee`（DB + 清理 profile 文件） |
...
| `TeamService` | `team_service.py` | legacy / experimental 多 Agent 协作；路由从 OpenAPI 主面隐藏 |
