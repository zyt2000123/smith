...
**流程：**
1. 生成 ID：`uuid4().hex[:8]`
2. 在数据库中创建Agent记录（`device` 默认为当前主机名）
...
更新Agent信息。仅更新请求体中提供的字段，未提供的字段保持不变。
...
删除Agent。同时删除数据库记录和 `~/.agent-smith/employees/<id>/` 目录下的所有文件。
...
操作Agent实例目录（`~/.agent-smith/employees/<id>/`）下的配置文件。
...
**消息路由逻辑：**
1. 保存用户消息
2. 提取消息中的 `@mentions`（匹配Agent名称）
3. 如果有 `@mentions`，仅路由到被提及的Agent
4. 如果没有 `@mentions`，路由到群组中的所有成员
5. 每个被路由的Agent独立调用 `engine.reply()` 生成回复
6. 返回所有回复消息
...
1. 从四层配置合并 LLM 参数（平台级 -> 模板级 -> Agent级 -> 会话级）
2. 加载工具/技能注册表
3. 组装 system prompt
...
```
环境变量 → 平台配置 (~/.agent-smith/config.yaml) → 模板配置 → Agent配置 → 会话覆盖
```
