...
llm:
  model: null  # null 表示继承上层配置（平台级 → 模板级 → 员工级 → 会话级）
knowledge:
  - api-design
  - database-modeling
  - authentication
...
`engine/skill/store.py` 中的 `SkillStore` 为每个员工技能维护版本快照：
...
| 操作 | 必填参数 | 说明 |
|------|---------|------|
| `list` | `employee_id` | 列出所有技能（内置 + 员工），含来源标记 |
| `get` | `employee_id`, `skill_name` | 读取技能内容（优先员工版本，其次内置） |
| `create` | `employee_id`, `skill_name`, `content` | 创建新员工技能（不可与内置技能同名） |
| `edit` | `employee_id`, `skill_name`, `content` | 全文替换员工技能（自动存版本） |
