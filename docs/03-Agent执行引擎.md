...
```
reply(agent_id, name, user_message)
  │
...
System Prompt 是 Agent 的"认知基础"。PromptAssembler 从 9 个层级逐层构建，每层负责一个维度的上下文。
...
| 6 | `context.md` | 用户偏好（可被 UserPreferenceLearner 写入） | 空字符串，跳过 |
| 7 | `agents/output_style.md` | 输出格式规范（全局共享） | 空字符串，跳过 |
| 8 | `memory/` | 记忆条目 | 空字符串，跳过 |
| 9 | 运行时上下文 | agent_id/name 等环境信息 | 空字符串，跳过 |
...
```python
tools_text = self._read(agent_dir / "toolbox.md")  # 静态指南
...
`expertise.json`、`traits.json`、`pipeline.json` 已从 Smith 模板移除。领域能力、具体 SOP 和交付管线通过 `agents/skills/*/SKILL.md` 表达；PromptAssembler 默认只注入 skill 摘要，具体 skill body 在执行对应 skill 时加载。
...
`reply(agent_id="emp-001", name="小张", user_message="帮我修复登录页的表单验证bug")`
...
│   ├── registry.py              # 技能注册表 (SkillRegistry + builtin/agent 分离)
