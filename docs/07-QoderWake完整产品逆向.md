...
> 性质：本文件是 QoderWake 逆向工程的最终汇总，整合 05（技术调研）和 06（Agent文件与技能系统）的全部发现，并补充此前未单独成文的深层分析
...
四款产品共享 Qoder 积分体系和统一订阅。QoderWake 是最新推出的产品线，定位从"编码辅助"拔高到"Agent"。
...
每个 Worker 在 `.qoder/` 下有 **9 个身份文件**，构成完整的Agent人格定义：
...
| 自主执行默认 | 默认做，不默认问 | Agent应该像真人一样执行 |
...
| WakerFlow 多 Agent | v0 先做单Agent，v1+ 再做多Agent | 先打通单Agent闭环 |
