...
1. **功能入口**（paddingTop 44pt）— "员工总览" / "定时任务"，选中态 `AppPalette.selectedSurface` 填充
2. **Divider**
3. **ScrollView**
...
所有员工的 `device` 均为 `"AA01030deMacBook-Pro.local"`。头像图片分别为 `product-manager.png`、`frontend-engineer.png`、`backend-engineer.png`，存放于 `Resources/Employees/` 目录。
...
| `selectedSegment` | `@State Int` | 顶部 tab（我的员工/我的群组） |
...
- **布局**：ScrollView -> 标题（"我的数字员工" + "新建员工"按钮） -> 筛选栏（Segmented Picker + 状态/环境下拉菜单 + 搜索框） -> 2 列 `LazyVGrid` 员工卡片
...
- **侧栏**：返回按钮（"我的员工"） -> 头像（48x58） + 姓名 + 在线状态 -> 标签页列表（选中蓝色高亮 `blue.opacity(0.12)`，hover `mutedSurface`）
