...
1. **引入时间分层编译**：在 `engine/memory/` 下新增 `compile.py`，实现 `compile_today()` / `compile_week()` / `compile_longterm()` / `compile_facts()` 四个函数，各自输出到 Agent 目录下的 `memory/today.md` 等文件
...
**核心差距**：
- 团队群聊中 N 个 Agent 收到消息时，遍历的是全部成员而非关心该事件的成员
...
3. **改造 `TeamService`**：发消息时 `emit(MESSAGE_RECEIVED, session_path=group_id)`，只唤醒订阅了该群组的 Agent，而非遍历全部成员
...
for emp_id in targets:
    reply = await engine_reply(emp_id, ...)  # 阻塞等每个 Agent 回复
    messages.append(reply)
```

**核心差距**：
- Agent 逐个串行回复，3 个 Agent = 3x 延迟
