...
IDENTITY 定义"我是谁"，PERSONA 定义"我怎么工作"，BIBLE 定义"我按什么流程交付"。三者组合构成完整的 Agent 人格。BIBLE 最复杂（约 600 行），包含任务路由、强制技能链、公共步骤（Understand → Investigate → Implement → Verify → Deliver），每步有 Gate / Output format / Backtrack trigger。
...
每 Agent 一个 Plugin，引用技能路径和 MCP 服务器配置。
