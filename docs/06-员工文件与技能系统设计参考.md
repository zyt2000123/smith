...
> 目的：为 Agent-Smith 的员工身份定义、技能打包、安全护栏、Hooks 系统提供设计参考
...
IDENTITY 定义"我是谁"，PERSONA 定义"我怎么工作"，BIBLE 定义"我按什么流程交付"。三者组合构成完整的员工人格。BIBLE 最复杂（约 600 行），包含任务路由、强制技能链、公共步骤（Understand → Investigate → Implement → Verify → Deliver），每步有 Gate / Output format / Backtrack trigger。
...
每员工一个 Plugin，引用技能路径和 MCP 服务器配置。
