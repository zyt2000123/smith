...
- **具体改动**:
  1. 实现 `compile_today()` / `compile_week()` / `compile_longterm()` / `compile_facts()` 四个函数，各自输出到员工目录 `memory/today.md` 等
...
- **检查要点**:
  ...
  - [ ] 技能自进化的评分阈值是否合理
  - [ ] 技能版本管理是否支持回滚
  - [ ] 员工自装技能 (`~/.agent-smith/employees/<id>/skills/`) 与内置技能的优先级
  - [ ] 技能依赖关系是否被正确处理
...
- **检查要点**:
  - [ ] 员工顺序回复的延迟是否可接受（3 个员工 = 3x LLM 调用时间）
